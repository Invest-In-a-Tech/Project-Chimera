...
class DataPipelineRunner:
    ...
    def __init__(self, config: Dict[str, Any], mode: PipelineMode = PipelineMode.AUTO) -> None:
        ...
        self.logger.info(
            "DataPipelineRunner initialized - Mode: %s, Config keys: %s",
            self.mode.value,
            list(config.keys()),
        )
        ...
        self.logger.debug(
            "Initializing DataPipelineRunner instance at memory address: %s",
            id(self),
        )
    ...
    def process_data(self) -> None:
        ...
        try:
            ...
            if effective_mode == PipelineMode.TRAINING:
                ...
                # This should always be true due to _validate_mode_config(),
                # but check for type safety
                # Type checker needs explicit confirmation that file_path is str
                # Keep explicit to avoid str | None ambiguity for static analysis
                if self.file_path is None:
                    # This should never happen if validation worked correctly
                    # Raise error to maintain type safety and catch configuration bugs
                    raise ValueError(
                        "TRAINING mode requires file_path but it is None"
                    )

                # Log the file being processed for audit trail
                # Useful for debugging file path issues or tracking data sources
                self.logger.info(
                    "Training mode: Processing data from file path: %s",
                    self.file_path,
                )
